...
function decodeProjectPath(dirName) {
  ...
  // The first segment after splitting would be empty (leading /)
  ...
  try {
    ...
    for (const line of lines) {
      try {
        ...
        // Extract session name if renamed
  ...
  try {
    ...
    for (const projDir of projectDirs) {
      ...
      for (const file of files) {
        // Only process main session files (not summaries)
...
// POST /api/sessions/:id/clone - Open in new terminal
app.post('/api/sessions/:id/clone', (req, res) => {
  const { id } = req.params;
  const { projectPath } = req.body;

  // Use osascript to open Terminal.app on macOS
